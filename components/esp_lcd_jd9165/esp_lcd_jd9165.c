...
#define JD9165BA_CMD_GS_BIT       (1 << 0)
#define JD9165BA_CMD_SS_BIT       (1 << 1)
...
//  {cmd, { data }, data_size, delay_ms}
    ...
    {0x3A, (uint8_t[]){0x77},1,0},  /* 0x77 = 24bpp RGB888 (was 0x55=16bpp RGB565) */
...
static esp_err_t panel_jd9165_init(esp_lcd_panel_t *panel)
{
    ...
    /* Skip panel ID read (command 0x04): many DSI panels don't support
     * reverse-direction LP reads, causing HAL to deadlock on RX FIFO.
     * Panel init sequence (write-only) works fine without this check. */
    ESP_LOGI(TAG, "Skipping panel ID read (DSI RX not supported by this panel)");
    (void)io;

    ESP_LOGI(TAG, "Sending MADCTL (0x%02X)...", jd9165->madctl_val);
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, LCD_CMD_MADCTL, (uint8_t[]) {
        jd9165->madctl_val,
    }, 1), TAG, "send command failed");
    ESP_LOGI(TAG, "MADCTL sent OK");
    ...
    for (int i = 0; i < init_cmds_size; i++) {
        ...
        // Send command
        ESP_LOGD(TAG, "Init [%d/%d]: cmd=0x%02X, data[0]=0x%02X, bytes=%d",
                 i, init_cmds_size, init_cmds[i].cmd,
                 init_cmds[i].data_bytes > 0 ? ((uint8_t*)init_cmds[i].data)[0] : 0,
                 (int)init_cmds[i].data_bytes);
        ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, init_cmds[i].cmd, init_cmds[i].data, init_cmds[i].data_bytes), TAG, "send command failed");
        /* 至少给 1ms 让面板处理命令, 避免 FIFO 堆积导致 ACK 丢失 */
        uint32_t dly = init_cmds[i].delay_ms;
        if (dly == 0) dly = 1;
...
static esp_err_t panel_jd9165_reset(esp_lcd_panel_t *panel)
{
    ...
    if (jd9165->reset_gpio_num >= 0)
    {
        ESP_LOGI(TAG, "Hardware reset DSI panel");
        gpio_set_level(jd9165->reset_gpio_num, !jd9165->flags.reset_level);
        vTaskDelay(pdMS_TO_TICKS(5));//120
        gpio_set_level(jd9165->reset_gpio_num, jd9165->flags.reset_level);
    ...
    { 
        ESP_LOGI(TAG, "Software reset DSI panel");
...
static esp_err_t panel_jd9165_mirror(esp_lcd_panel_t *panel, bool mirror_x, bool mirror_y)
{
    ...
    if (mirror_x) {
        madctl_val |= JD9165BA_CMD_GS_BIT;
    } else {
        madctl_val &= ~JD9165BA_CMD_GS_BIT;
    }
    if (mirror_y) {
        madctl_val |= JD9165BA_CMD_SS_BIT;
    } else {
        madctl_val &= ~JD9165BA_CMD_SS_BIT;
    }
